...
class AsrGoogleComp : public RoboComp::Application
{
private:
...
int AsrGoogleComp::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
    ...
    while(!asrpublish_topic){
		try {
			asrpublish_topic = topicManager->create(communicator()->getProperties()->getProperty("ASRPublish"));
		}catch (const IceStorm::TopicExists&){
		  	// Another client created the topic.
			try{
				asrpublish_topic = topicManager->retrieve(communicator()->getProperties()->getProperty("ASRPublish"));
