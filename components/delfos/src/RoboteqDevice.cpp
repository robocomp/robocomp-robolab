...
int RoboteqDevice::SetConfig(bool CAN, int configItem, int index, int value)
{
	string response;
	char command[10];
	char args[50];
	...
	if(index < 0)
        ...
        if (CAN)
            status = IssueCommand("@02^", command, args, 10, response, true);
        else
            status = IssueCommand("^", command, args, 10, response, true);
...
int RoboteqDevice::SetConfig(bool CAN, int configItem, int value)
{
	return SetConfig(CAN, configItem, MISSING_VALUE, value);
}

int RoboteqDevice::SetCommand(bool CAN, int commandItem, int index, int value)
{
	string response;
	char command[10];
	char args[50];
...
int RoboteqDevice::SetCommand(bool CAN, int commandItem, int value)
{
	return SetCommand(CAN, commandItem, MISSING_VALUE, value);
}
int RoboteqDevice::SetCommand(bool CAN, int commandItem)
{
	return SetCommand(CAN, commandItem, MISSING_VALUE, MISSING_VALUE);
}

int RoboteqDevice::GetConfig(bool CAN, int configItem, int index, int &result)
{
	string response;
	char command[10];
	char args[50];
	...
	sprintf(args, "%i", index);
        if (CAN)
            status = IssueCommand("@02~", command, args, 10, response);
        else
            status = IssueCommand("~", command, args, 10, response);
	if(status != RQ_SUCCESS)
		return status;
...
int RoboteqDevice::GetConfig(bool CAN, int configItem, int &result)
{
	return GetConfig(CAN, configItem, 0, result);
}

int RoboteqDevice::GetValue(bool CAN, int operatingItem, int index, int &result)
{
	string response;
	char command[10];
	char args[50];
...
int RoboteqDevice::GetValue(bool CAN, int operatingItem, int &result)
{
	return GetValue(CAN, operatingItem, 0, result);
}
