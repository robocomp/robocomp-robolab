...
Ice.loadSlice("-I ./src/ --all ./src/BodyHandJointsDetector.ice")
import RoboCompBodyHandJointsDetector
...
setattr(RoboCompBodyHandJointsDetector, "ImgType", ImgType)
class Point(list):
    def __init__(self, iterable=list()):
    ...
    def append(self, item):
        ...
        super(Point, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, int)
        super(Point, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, int)
        super(Point, self).insert(index, item)

setattr(RoboCompBodyHandJointsDetector, "Point", Point)
class CoordSequence(list):
    def __init__(self, iterable=list()):
        super(CoordSequence, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompBodyHandJointsDetector.Point)
        super(CoordSequence, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompBodyHandJointsDetector.Point)
        super(CoordSequence, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompBodyHandJointsDetector.Point)
        super(CoordSequence, self).insert(index, item)

setattr(RoboCompBodyHandJointsDetector, "CoordSequence", CoordSequence)
class ListHands(list):
    def __init__(self, iterable=list()):
        super(ListHands, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompBodyHandJointsDetector.THand)
        super(ListHands, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompBodyHandJointsDetector.THand)
        super(ListHands, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompBodyHandJointsDetector.THand)
        super(ListHands, self).insert(index, item)

setattr(RoboCompBodyHandJointsDetector, "ListHands", ListHands)
class ListFullBody(list):
    def __init__(self, iterable=list()):
        super(ListFullBody, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompBodyHandJointsDetector.TFullBody)
        super(ListFullBody, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompBodyHandJointsDetector.TFullBody)
        super(ListFullBody, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompBodyHandJointsDetector.TFullBody)
        super(ListFullBody, self).insert(index, item)

setattr(RoboCompBodyHandJointsDetector, "ListFullBody", ListFullBody)

import bodyhandjointsdetectorI
...
class Requires:
    def __init__(self, ice_connector):
        """
        init connection for required components
        :param ice_connector:
        """
        self.ice_connector = ice_connector
        self.mprx={}
    ...
    def create_proxy(self, property_name, ice_proxy):
        # Remote object connection for required components
        try:
...
class Implements:
    def __init__(self, ice_connector, default_handler):
        self.ice_connector = ice_connector
        self.bodyhandjointsdetector = self.create_adapter("BodyHandJointsDetector", bodyhandjointsdetectorI.BodyHandJointsDetectorI(default_handler))
...
class InterfaceManager:
    def __init__(self, ice_config_file):
        ...
        # set param for other required components
    ...
    def get_proxies_map(self):
        # get all proxies which are needed
        result = {}
