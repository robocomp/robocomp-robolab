...
def computePilar(self):
            ...
            for j in range(len(keypoints)):
                if keypoints[j][1]:
                    ...
                    if self.draw:
                        ...
                        cv2.putText(self.skeleton_img, "%d" % int(keypoints[j][0]), (int(x) + 5, int(y)),
    ...
    def c2d(p):
        ...
        depth_point = rs.rs2_project_color_pixel_to_depth_pixel(depth_frame.get_data(), depth_scale, depth_min,
                                                                depth_max, depth_intrin, color_intrin,
                                                                depth_to_color_extrin, color_to_depth_extrin, [x, y])
    ...
    def d2xyz(depth_point):
        ...
        if np.any(depth_point == None):
            return False, [0, 0, 0]
        try:
            ret = True, rs.rs2_deproject_pixel_to_point(depth_intrin, depth_point,
                                                        depth_scale * depth_data[depth_point[1], depth_point[0]])
            return ret
        except IndexError:
            return False, [0, 0, 0]
        ...
        for kp in range(len(keypoints)):
            ...
            if centre[1] and centre[2]:
                ...
                if self.draw:
                    ...
                    vector2 /= vector2[2][0]
                    try:
                        cv2.circle(self.skeleton_img, (int(vector2[0][0]) - 1, int(vector2[1][0]) - 1), 1,
                                   (0, 100, 255), 2)
                    except:
                        pass
