...
class NumPyData(list):
    def __init__(self, iterable=list()):
        super(NumPyData, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, byte)
        super(NumPyData, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, byte)
        super(NumPyData, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, byte)
        super(NumPyData, self).insert(index, item)

setattr(RoboCompPoseBasedGestureRecognition, "NumPyData", NumPyData)
