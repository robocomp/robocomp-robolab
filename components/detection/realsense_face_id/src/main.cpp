...
/** \mainpage RoboComp::realsense_face_id
 *
 * \section intro_sec Introduction
 *
 * The realsense_face_id component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/realsense_face_id --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class realsense_face_id : public RoboComp::Application
{
public:
	realsense_face_id (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
void ::realsense_face_id::initialize()
{
...
int ::realsense_face_id::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		...
		{
			// Server adapter creation and publication
			if (not GenericMonitor::configGetString(communicator(), prefix, "RealSenseFaceID.Endpoints", tmp, ""))
			{
				cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy RealSenseFaceID";
			}
			Ice::ObjectAdapterPtr adapterRealSenseFaceID = communicator()->createObjectAdapterWithEndpoints("RealSenseFaceID", tmp);
			auto realsensefaceid = std::make_shared<RealSenseFaceIDI>(worker);
			adapterRealSenseFaceID->add(realsensefaceid, Ice::stringToIdentity("realsensefaceid"));
			adapterRealSenseFaceID->activate();
			cout << "[" << PROGRAM_NAME << "]: RealSenseFaceID adapter created in port " << tmp << endl;
		}
		catch (const IceStorm::TopicExists&){
			cout << "[" << PROGRAM_NAME << "]: ERROR creating or activating adapter for RealSenseFaceID\n";
		}
...
int main(int argc, char* argv[])
{
	...
	::realsense_face_id app(prefix, startup_check_flag);
