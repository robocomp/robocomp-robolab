...
for i, img_file in enumerate(img_list):
    ...
    if args.pretrained == 'KITTI':
        ...
        out2 = deepcopy(out)
        out = out*1000.0
    # out2 = F.interpolate(out2, (1080, 1920), mode='bilinear')
    out = out.cpu().detach().numpy().astype(np.uint16)
    out2 = out2.cpu().detach().numpy().astype(np.uint16)
    out3 = cv2.resize(out2, dsize=(1080,3840))
    # print(out2.shape)
    result_filename = result_filelist[i]
    np.save('.'+result_filelist[i].split('.')[1]+'.npy', out3)
    out = (out/out.max())*255.0
    # if i == 1:
        # print(result_filelist[1].split('.')[1])
