...
/** \mainpage RoboComp::multiple_cameras
 *
 * \section intro_sec Introduction
 *
 * The multiple_cameras component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/multiple_cameras --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class multiple_cameras : public RoboComp::Application
{
public:
	multiple_cameras (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
void ::multiple_cameras::initialize()
{
...
int ::multiple_cameras::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	while (!laserpub_topic)
	{
		...
		{
			cout << "[" << PROGRAM_NAME << "]: ERROR retrieving LaserPub topic. \n";
			try
			{
				...
				// Another client created the topic.
				cout << "[" << PROGRAM_NAME << "]: ERROR publishing the LaserPub topic. It's possible that other component have created\n";
			}
		}
		catch(const IceUtil::NullHandleException&)
		{
	...
	auto laserpub_pub = laserpub_topic->getPublisher()->ice_oneway();
	laserpub_pubproxy = Ice::uncheckedCast<RoboCompLaserPub::LaserPubPrx>(laserpub_pub);

	tprx = std::make_tuple(camerargbdsimplepub_pubproxy,laserpub_pubproxy);
...
int main(int argc, char* argv[])
{
	...
	::multiple_cameras app(prefix, startup_check_flag);
