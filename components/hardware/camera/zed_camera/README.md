# zed_camera
RoboComp component that reads an RGBD stream from a Stereolabs ZED camera (real hardware)
or from a Webots simulation (through a `CameraRGBDSimple` proxy) and republishes it.

It:
- implements `CameraRGBDSimple` (image, depth, points, getAll) and `Lidar3D` (color point cloud);
- publishes RGBD frames on `CameraRGBDSimplePub`, and (optionally) pose on `FullPoseEstimationPub`
  and IMU on `IMUPub`;
- reprojects depth into a 3D point cloud, applying a configurable camera→robot extrinsic transform.

The `Config.Simulated` flag selects the source: `true` pulls frames from the `CameraRGBDSimple`
proxy (Webots), `false` opens a physical ZED camera via the ZED SDK.

## Dependencies
The following dependencies are required to build and run zed_camera:
```
- RoboComp core (ROBOCOMP env var set)
- ZED SDK 3.x + CUDA (only needed when Config.Simulated = false)
- OpenCV 4
- Eigen 3
- OpenMP
- zstd (libzstd-dev, used by CloudCompressor)
- Qt6 + Ice (pulled in by the RoboComp component skeleton)
- eProsima Fast DDS + Fast CDR (for the optional zero-copy DDS media plane)
```

## DDS media plane (CORTEX)

Besides the Ice interfaces, the component can publish RGB + depth on a zero-copy
Fast DDS "media plane" (shared lib `active_inference/common/media_transport`), so
CORTEX agents (e.g. the voxelizer) attach with SHM data-sharing. It is **gated by
config** and off by default:

```toml
[Config]
PublishDDS = true      # enable the media plane

[DDS]
Domain     = 7         # CORTEX media domain
RGBTopic   = "rc/zed/rgb"
DepthTopic = "rc/zed/depth"
```

Camera **metadata** (intrinsics, media-plane discovery descriptor) is NOT written
here — the `robot_concept` DSR agent advertises it on the robot node. This
component only moves pixels.

## Configuration parameters
Like any other component, zed_camera requires a configuration file to start. In etc/config or etc/config.toml, you can find an example of the configuration file.
...
bin/zed_camera etc/yourConfig # Execute the component
```
-----
-----
# Developer Notes
This section explains how to work with the generated code of zed_camera, including what can be modified and how to use key features.
## Editable Files
