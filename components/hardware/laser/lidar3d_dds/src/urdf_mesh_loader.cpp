...
bool URDFMeshLoader::parseLink(void *xml_element) {
  ...
  if (!geom_parent_xml)
    geom_parent_xml = link_xml->FirstChildElement("visual");

  if (geom_parent_xml) {
    XMLElement *origin_xml = geom_parent_xml->FirstChildElement("origin");
    link->visual_origin = parseOrigin(origin_xml);
...
bool URDFMeshLoader::loadURDF(const std::string &urdf_path,
  ...
  if (doc.LoadFile(urdf_path.c_str()) != XML_SUCCESS) {
    std::cerr << "URDFMeshLoader: Failed to load URDF " << urdf_path
              << std::endl;
    return false;
  }
  ...
  for (XMLElement *el = robot->FirstChildElement(); el != nullptr;
  ...
  for (auto &pair : m_joints) {
    ...
    if (j->parent_link)
      j->parent_link->children.push_back(j);
    if (j->child_link) {
  ...
  std::map<std::string, float> empty_angles;
  updateJoints(empty_angles);

  std::cout << "URDF Loaded successfully. Root link: "
            << (m_root_link ? m_root_link->name : "NONE") << std::endl;
  return true;
}
...
void URDFMeshLoader::updateJoints(
    const std::map<std::string, float> &joint_angles) {
  if (!m_root_link)
    return;
...
void URDFMeshLoader::computeFK(
    LinkNode *link, const Eigen::Matrix4f &parent_transform,
    const std::map<std::string, float> &joint_angles) {
    ...
    if (link->parent_joint) {
        ...
        if (joint_angles.count(j->name)) {
        float angle = joint_angles.at(j->name);
        // std::cout << "Joint name: " << j->name << std::endl;
        // std::cout << "Joint type: " << j->type << std::endl;
        // std::cout << "Joint origin: " << j->origin << std::endl;
        // std::cout << "Joint axis: " << j->axis << std::endl;
        // std::cout << "Joint parent link: " << j->parent_link << std::endl;
        // std::cout << "Joint child link: " << j->child_link << std::endl;
        ...
        if (link->has_geometry) {
        RTCGeometry geom = rtcGetGeometry(m_scene, link->embree_instance_id);
        Eigen::Matrix4f mesh_transform = current_transform * link->visual_origin;
        rtcSetGeometryTransform(geom, 0, RTC_FORMAT_FLOAT4X4_COLUMN_MAJOR,
                                mesh_transform.data());
        rtcCommitGeometry(geom);
        }
