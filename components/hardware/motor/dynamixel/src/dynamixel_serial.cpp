...
Dynamixel::Dynamixel(  RoboCompJointMotorSimple::MotorParams params, QMutex *m)
{
	h_mutex = new QMutex();
	m_mutex = new QMutex();
	this->params = params;
...
/*	if (counter == 2)  qDebug()<<"||  DYNAMIXEL::initialize -----> USB CONNECTED!!  ||";
	...
	if (port.open(QIODevice::ReadWrite) == false)
	{
		...
		QString error = "||  ERROR DYNAMIXEL::initialize ---> Port " + device +
...
//	}
    ...
    if(port.baudRate() !=  QSerialPort::Baud115200)
        qFatal("ERROR DYNAMIvXEL::initialize!! ---> Error setting Baud Rate %d\n   ||", 115200);;

	//4) Create servos instances in a QMap indexed by name
	servo = new Servo( params );

//	servo->setMotorRanges(params->operator[](i).stepsRange,params->operator[](i).maxDegrees, 1024, 12);
//	}
//
...
/* Output : Position                                                          */
/******************************************************************************/
void Dynamixel::getPosition(const QString &motor, float  &pos)
{
...
void Dynamixel::setPosition( const QString &motor,  float  pos, float maxSpeed )
{
	QMutexLocker locker(h_mutex);
...
void Dynamixel::setSyncPosition( const QVector<Dynamixel::GoalPosition> & goals)
{
	QMutexLocker locker(h_mutex);
