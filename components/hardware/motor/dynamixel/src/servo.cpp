...
Servo::Servo( RoboCompJointMotorSimple::MotorParams params )
{
  this->params = params;
...
{
	//Max speed
	int res = (int)rint((RAW_SPEED_STEPS_RANGE / MAX_SPEED_RADS) * rs );
	if (res > 1024) return 1024;
	if (res < -1024) return -1024;
	return res;
}
float Servo::stepsPerSec2Rads(int p)
{
	return (float)(p * MAX_SPEED_RADS / RAW_SPEED_STEPS_RANGE);
}
