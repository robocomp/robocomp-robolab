...
{
    // Solo reaccionamos si es clic izquierdo Y tenemos acceso al worker
   	if (g_worker) g_worker->handleMouse(event, x, y);
}

void SpecificWorker::handleMouse(int event, int x, int y)
{
...
void SpecificWorker::compute()
{
    ...
    try
    {
        ...
        if(t_lidar == 0) t_lidar = mrpt::system::now_double();
        ...
        obsScan->timestamp = mrpt::system::time_tToTimestamp(t_lidar);
...
void SpecificWorker::updateVis()
{
    ...
    if (!windowCreated) {
        // WINDOW_NORMAL permite que el usuario cambie el tamaño con el ratón
        cv::namedWindow("Navegacion", cv::WINDOW_NORMAL);
        // Establecemos un tamaño inicial por defecto
        cv::resizeWindow("Navegacion", 1000, 800);
        windowCreated = true;
    }
    ...
    // --- CAMBIO CLAVE: Preguntar el tamaño actual de la ventana ---
    // getWindowImageRect devuelve el rectángulo del área cliente de la ventana
    cv::Rect r = cv::getWindowImageRect("Navegacion");
    int winW = r.width;
    int winH = r.height;

    // Protección para el primer frame o si se minimiza
    if (winW <= 0 || winH <= 0) {
        winW = 1000; winH = 800;
    }

    // Usamos ese tamaño dinámico como destino
    cv::Mat displayImg;
    cv::Mat T = (cv::Mat_<double>(2,3) << view_scale, 0, view_pan_x,
                                          0, view_scale, view_pan_y);

    // warpAffine rellenará exactamente el tamaño que tenga la ventana en este momento
    cv::warpAffine(cvImg, displayImg, T, cv::Size(winW, winH), cv::INTER_NEAREST, cv::BORDER_CONSTANT, cv::Scalar(128,128,128));
    ...
    cv::imshow("Navegacion", displayImg);
...
void SpecificWorker::FullPoseEstimationPub_newFullPose(RoboCompFullPoseEstimation::FullPoseEuler pose)
{
    ...
    v.vy = -pose.vx;
    v.w  = -pose.vrz;
    v.timestamp_seconds = (double)pose.timestamp / 1000.0;
