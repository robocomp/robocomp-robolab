	...
	for (i = 0; i < cnt; ++i)
	{
		...
		if (desc.idVendor == MS_MAGIC_VENDOR && desc.idProduct == MS_MAGIC_CAMERA_PRODUCT && !start_cam)
		{
			...
			if (nr_cam == index)
			{
				...
				if (!dev->usb_cam.dev) 
					return (-1);
				int err = libusb_claim_interface (dev->usb_cam.dev, 0);
        if(err!=0){
          printf("Error claiming camera device: ");
		...
		if (desc.idVendor == MS_MAGIC_VENDOR && desc.idProduct == MS_MAGIC_MOTOR_PRODUCT && !start_motor)
		{
			...
			if (nr_mot == index)
			{
				...
				if (!dev->usb_motor.dev)
					return (-1);
				int err = libusb_claim_interface (dev->usb_motor.dev, 0);
        if(err!=0){
          printf("Error claiming motor device: ");
          if(err==LIBUSB_ERROR_NOT_FOUND){
            printf("LIBUSB_ERROR_NOT_FOUND\n");
          }else if(err==LIBUSB_ERROR_BUSY){
            printf("LIBUSB_ERROR_BUSY\n");
...
int fnusb_start_iso(fnusb_dev *dev, fnusb_isoc_stream *strm, fnusb_iso_cb cb, int ep, int xfers, int pkts, int len)
{
    ...
    if(ret==LIBUSB_ERROR_NO_DEVICE)
      printf("Failed to submit xfer %d: LIBUSB_ERROR_NO_DEVICE\n",i);
    else if(ret==LIBUSB_ERROR_BUSY)
      printf("Failed to submit xfer %d: LIBUSB_ERROR_BUSY\n",i);
    else if (ret < 0)
