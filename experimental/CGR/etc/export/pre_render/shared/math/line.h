...
#ifdef LazyCaching
  inline const GVector::vector2d<num>& Dir() const {if(updateRequired) calcValues(); return dir;}
  inline const GVector::vector2d<num>& Perp() const {if(updateRequired) calcValues(); return perp;}
  inline const num Length() const {if(updateRequired) calcValues(); return length;}
  inline const num Angle() const {if(updateRequired) calcValues(); return dir.angle();}
#else
...
LINE_TEM
LINE_FUN::Line2d(GVector::vector2d< num > _p1)
{
  p0.zero(); 
  p1 = _p1;
#ifdef LazyCaching
...
LINE_TEM
LINE_FUN::Line2d(GVector::vector2d< num > _p0, GVector::vector2d< num > _p1)
{
  p0 = _p0; 
  p1 = _p1;
#ifdef LazyCaching
...
LINE_TEM
void LINE_FUN::set(GVector::vector2d<num> _p0, GVector::vector2d<num> _p1)
{
  p0 = _p0; 
  p1 = _p1;
#ifdef LazyCaching
...
LINE_TEM
num LINE_FUN::distFromLine(GVector::vector2d<num> p, GVector::vector2d<num> rayDir, bool extendLine)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
...
LINE_TEM
num LINE_FUN::distFromLine1(GVector::vector2d<num> p, num rayDir, bool extendLine)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
...
LINE_TEM
GVector::vector2d<num> LINE_FUN::perpFromLine(GVector::vector2d<num> p, bool endcaps)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
...
LINE_TEM
bool LINE_FUN::liesAlongside(GVector::vector2d<num> p, num margin)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
...
LINE_TEM
GVector::vector2d<num> LINE_FUN::intersectTest(GVector::vector2d< num > p2, GVector::vector2d< num > p3, bool &intersects, bool extendL0, bool extendL1)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
  ...
  intersects = true;
  if( (!extendL0 && (position0<0.0 || position0>length)) || (!extendL1 && (sq(position1)>(p3-p2).sqlength())) )
    intersects = false;
...
LINE_TEM
bool LINE_FUN::intersects(GVector::vector2d< num > p, GVector::vector2d< num > rayDir, bool touching)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
  ...
  if(touching)
    return ( v0.cross(rayDir)>=0.0 && v1.cross(rayDir) <= 0.0 ); //Check if rayDir lies within the angle reqion between v0 and v1
  else
    return ( v0.cross(rayDir)>eps && v1.cross(rayDir) < -eps ); //Check if rayDir lies within the angle reqion between v0 and v1
}

LINE_TEM
bool LINE_FUN::intersects(GVector::vector2d<num> p, num rayDir)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
...
LINE_TEM
GVector::vector2d<num> LINE_FUN::intersection(Line2d< num > l1, bool extendL0, bool extendL1)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
  ...
  if( (!extendL0 && (position0<0.0 || position0>length)) || (!extendL1 && (position1<0.0 || position1>l1.Length())) )
    return GVector::vector2d<num>(NAN,NAN);
...
LINE_TEM
Line2d<num> LINE_FUN::rotate(GVector::vector2d<num> p, num angle)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
...
LINE_TEM
GVector::vector2d<num> LINE_FUN::project_in(GVector::vector2d<num> p)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
...
LINE_TEM
GVector::vector2d<num> LINE_FUN::project_out(GVector::vector2d<num> p)
{
  #ifdef LazyCaching
  if(updateRequired) calcValues();
  #else
...
#define LINE_BINARY_OPERATOR_VECTOR(op) \
  LINE_TEM \
  Line2d<num> LINE_FUN::operator op (GVector::vector2d<num> v) const\
  {                   \
...
#define LINE_UNARY_OPERATOR_VECTOR(op) \
  LINE_TEM \
  Line2d<num>& LINE_FUN::operator op (GVector::vector2d<num> v) \
  {                   \
    p0 = p0 op v;   \
    p1 = p1 op v;   \
    updateRequired = true;     \
