...
/** \mainpage RoboComp::missionPlanner
 *
 * \section intro_sec Introduction
 *
 * The missionPlanner component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/missionPlanner --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class missionPlanner : public RoboComp::Application
{
public:
	missionPlanner (QString prfx) { prefix = prfx.toStdString(); }
private:
	void initialize();
	std::string prefix;
	MapPrx mprx;
...
IceStorm::TopicManagerPrx topicManager = IceStorm::TopicManagerPrx::checkedCast(communicator()->propertyToProxy("TopicManager.Proxy"));
	...
	IceStorm::TopicPrx asrpublish_topic;
	while (!asrpublish_topic)
	{
		try
		{
		...
		catch (const IceStorm::NoSuchTopic&)
		{
			try
			{
				asrpublish_topic = topicManager->create("ASRPublish");
			}
	...
	Ice::ObjectPrx asrpublish_pub = asrpublish_topic->getPublisher()->ice_oneway();
	ASRPublishPrx asrpublish = ASRPublishPrx::uncheckedCast(asrpublish_pub);
	mprx["ASRPublishPub"] = (::IceProxy::Ice::Object*)(&asrpublish);
