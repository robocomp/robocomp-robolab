 ...
 */
/** \mainpage RoboComp::primesenseComp
 *
 * \section intro_sec Introduction
 *
 * The primesenseComp component...
 *
 * \section interface_sec Interface
 *
 * primesenseComp interface...
 *
 ...
 *
 * <p>
 * The configuration file primesenseComp/etc/config ...
 * </p>
 *
 * \subsection execution_ssec Execution
 *
 * Just: "${PATH_TO_BINARY}/primesenseComp --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class PrimeSenseComp : public RoboComp::Application
{
private:
	void initialize();
public:
...
void PrimeSenseComp::initialize()
{
	// Config file properties read example
	// configGetString( PROPERTY_NAME_1, property1_holder, PROPERTY_1_DEFAULT_VALUE );
	configGetInt("FPS", fps, 30);
	printf("FPS: %d\n", fps);
}

int PrimeSenseComp::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	// DifferentialRobot server proxy creation example
	try
	{
		// Load the differentialrobot server proxy
	...
	// JointMotor server proxy creation example
	try
	{
		// Load the jointmotor server proxy
	...
	Worker *worker = new Worker(fps, differentialrobot_proxy, jointmotor_proxy);
	...
	try
	{
		...
		// Server adapter creation and publication
		Ice::ObjectAdapterPtr adapter = communicator()->createObjectAdapter("PrimeSenseComp");
		RGBDI *rgbdI = new RGBDI(worker );
		adapter->add(rgbdI, communicator()->stringToIdentity("rgbd"));
