...
void SpecificMonitor::readConfig(RoboCompCommonBehavior::ParameterList &params )
{
 	...
 	aux.editable = true;
 	configGetString("Visor3D.InnerModelPath", "", aux.value, "nofile");
	if( QFile::exists(aux.value.c_str()) == false )
	{
		qDebug() << __FUNCTION__ << "Warning, InnerModel file not found at" << aux.value.c_str();
		qFatal("Aborting. Cannor continue without an InnerModel file");
	}
	else
		params["Visor3D.InnerModelPath"] = aux;
