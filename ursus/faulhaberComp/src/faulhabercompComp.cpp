...
class faulhaberCompComp : public RoboComp::Application
{
private:
...
void faulhaberCompComp::initialize()
{
...
int faulhaberCompComp::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		...
		// Server adapter creation and publication
		Ice::ObjectAdapterPtr adapter = communicator()->createObjectAdapter("faulhaberComp");
		JointMotorI *jointmotor = new JointMotorI(worker);
		adapter->add(jointmotor, communicator()->stringToIdentity("jointmotor"));
